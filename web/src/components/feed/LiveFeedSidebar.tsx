      ...
      >
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
            ...
            <button
              ...
              style={{
                ...
                letterSpacing: '0.5px',
                textTransform: 'uppercase',
                color: paused ? '#f0a030' : '#38405a',
                background: paused ? 'rgba(240,160,48,0.08)' : 'none',
                border: `1px solid ${paused ? 'rgba(240,160,48,0.3)' : '#1a2030'}`,
                padding: '4px 9px',
                borderRadius: 3,
                cursor: 'pointer',
              }}
            ...
            {TABS.map((tab) => (
              <button
                ...
                style={{
                  ...
                  letterSpacing: '0.5px',
                  textTransform: 'uppercase',
                  color: activeTab === tab ? '#e2e6f0' : '#38405a',
                  background: 'none',
                  border: 'none',
                  borderBottom: `2px solid ${activeTab === tab ? '#00d68f' : 'transparent'}`,
                  padding: '8px 10px',
                  cursor: 'pointer',
                  marginBottom: -1,
                }}
        ...
        >
          {isAuthed ? (
            ...
            <div style={{ display: 'flex', gap: 10, alignItems: 'flex-start' }}>
              ...
              <div
                style={{
                  ...
                  borderRadius: '50%',
                  flexShrink: 0,
                  background: getAvatarGradient(user?.name || user?.email || 'user'),
                  display: 'flex',
                  alignItems: 'center',
                  justifyContent: 'center',
                  fontFamily: 'var(--font-dm-mono)',
              ...
              <div style={{ flex: 1 }}>
                <textarea
                  ...
                  placeholder="Share your read on the market..."
                  rows={2}
                  style={{
                    width: '100%',
                    background: 'none',
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 8 }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <span style={{ fontFamily: 'var(--font-dm-mono)', fontSize: 9, color: charsLeft < 20 ? (charsLeft < 0 ? '#f03e5a' : '#f0a030') : '#38405a' }}>
                      {charsLeft}
                    </span>
                    <button
                      onClick={submitPost}
                      disabled={!composeText.trim() || charsLeft < 0 || posting}
                      style={{
                        fontFamily: 'var(--font-dm-mono)',
                        fontSize: 9,
                        letterSpacing: '0.8px',
                        textTransform: 'uppercase',
                        color: '#070809',
                        background: composeText.trim() && charsLeft >= 0 ? '#00d68f' : '#1a2030',
                        border: 'none',
                        padding: '5px 14px',
                        borderRadius: 3,
                        cursor: composeText.trim() && charsLeft >= 0 ? 'pointer' : 'default',
                        fontWeight: 500,
                        transition: 'background 0.15s',
                      }}
                    >
            ...
            /* Guest locked state */
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <div style={{ width: 28, height: 28, borderRadius: '50%', flexShrink: 0, background: '#1a2030', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, color: '#38405a' }}>
                ?
              </div>
              <div style={{ flex: 1, fontFamily: 'var(--font-dm-sans)', fontSize: 11.5, color: '#788098', lineHeight: 1.5 }}>
                <a href="/auth" style={{ color: '#00d68f', textDecoration: 'none', fontWeight: 500 }}>Sign in</a>
                {' or '}
                <a href="/auth?mode=register" style={{ color: '#00d68f', textDecoration: 'none', fontWeight: 500 }}>join free</a>
                {' to share your market read'}
              </div>
            </div>
          )}
        </div>
        ...
        {showAuthBanner && (
          <div
            style={{
              ...
              flexShrink: 0,
              animation: 'postIn 0.3s ease forwards',
            }}
          >
            <span style={{ fontFamily: 'var(--font-dm-mono)', fontSize: 10, color: '#a0a8c0', lineHeight: 1.5 }}>
              <a href="/auth?mode=register" style={{ color: '#00d68f', textDecoration: 'none', fontWeight: 500 }}>Create a free account</a>
              {' to react, repost, and post.'}
            </span>
            <button
              onClick={() => setShowAuthBanner(false)}
              style={{ background: 'none', border: 'none', color: '#38405a', cursor: 'pointer', fontSize: 16, padding: '0 4px', lineHeight: 1, flexShrink: 0 }}
            >
              ×
            </button>
          </div>
        ...
        >
          ...
          {loading && (
            <div style={{ padding: '24px 16px', textAlign: 'center' }}>
              <span style={{ fontFamily: 'var(--font-dm-mono)', fontSize: 10, color: '#38405a', letterSpacing: '1px' }}>
                Loading feed…
              </span>
            </div>
          )}
          ...
          {!loading && posts.length === 0 && (
            <div style={{ padding: '32px 16px', textAlign: 'center' }}>
              <div style={{ fontFamily: 'Bebas Neue, var(--font-bebas)', fontSize: 20, color: '#38405a', letterSpacing: '2px', marginBottom: 8 }}>
                No posts yet
              </div>
              <p style={{ fontFamily: 'var(--font-dm-mono)', fontSize: 10, color: '#38405a' }}>
                Be the first to share your market read.
              </p>
            </div>
          )}
          ...
          {posts.map((post, idx) => (
            <div key={post.id}>
              ...
              />
              {/* Follow suggestion every 5 posts */}
              {(idx + 1) % 5 === 0 && idx < posts.length - 1 && (
                <FollowSuggestion idx={idx} />
              )}
            </div>
          ))}
        </div>
